...
class RAPIDModel(nn.Module):
        ...
        for i, (hist_entry, t) in enumerate(zip(history, history_t)):
            ...
            if t in self._rgcn_cache:
                ...
                if self._dgl_has_cuda and g.device != rgcn_device:
                    g = g.to(rgcn_device)
                if "id" in g.ndata:
                    node_features = self.entity_embeds[g.ndata["id"].view(-1)].to(
                        rgcn_device
                    )
                else:
    ...
    def forward(
        ...
        Uses oracle (ground-truth) histories.
    ...
    def _encode_history_batch(
        ...
        actual_lens = torch.LongTensor([max(length, 1) for length in actual_lens])
    ...
    def predict_batch(
        ...
        with torch.no_grad():
            for i in range(batch_size):
                entity1_temporal[i] = self._get_entity_temporal_embed(
                    entity1_ids[i].item()
                )
                entity2_temporal[i] = self._get_entity_temporal_embed(
                    entity2_ids[i].item()
                )
