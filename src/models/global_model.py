...
class GlobalRGCNAggregator(nn.Module):
    ...
    def _process_graph_batch(
        ...
        """Helper to batch and process graphs for a list of timesteps."""
        if not unique_times:
            return torch.zeros(0, self.hidden_dim, device=ent_embeds.device)
    ...
    def forward(
        ...
        if not any(len_non_zero):
            ...
            embed_seq = torch.zeros(
                len(t_list), 1, self.hidden_dim, device=ent_embeds.device
            )
        ...
        global_info = self._process_graph_batch(unique_times, ent_embeds, graph_dict)

        if global_info.size(0) == 0:
            embed_seq = torch.zeros(
                len(t_list), 1, self.hidden_dim, device=ent_embeds.device
            )
            return torch.nn.utils.rnn.pack_padded_sequence(
        ...
        embed_seq_tensor = torch.zeros(
            len(len_non_zero), max_len, self.hidden_dim, device=ent_embeds.device
        )
    ...
    def predict(
        ...
        # Process graphs
        global_info = self._process_graph_batch(selected_times, ent_embeds, graph_dict)
...
class PPIGlobalModel(nn.Module):
    ...
    def forward(
        ...
        if len(hidden) < len(t_list):
            padding = torch.zeros(
                len(t_list) - len(hidden), self.hidden_dim, device=hidden.device
            )
