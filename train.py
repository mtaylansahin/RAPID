...
def train(args):
    ...
    utils.set_cuda(use_cuda)  # Set global CUDA flag
    seed = 999
    np.random.seed(seed)
    ...
    while True:
        ...
        batch_iter = utils.make_batch2(train_data_shuffle, s_history_shuffle, s_history_t_shuffle,
                                       o_history_shuffle, o_history_t_shuffle, args.batch_size)
        num_batches = len(train_data) // args.batch_size + (1 if len(train_data) % args.batch_size else 0)
        pbar = tqdm(batch_iter, total=num_batches, desc=f"Epoch {epoch:04d}")
        for batch_data, s_hist, s_hist_t, o_hist, o_hist_t in pbar:
            # break
