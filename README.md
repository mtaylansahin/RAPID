...
This project uses [uv](https://docs.astral.sh/uv/) for dependency management. To set up the environment:
...
Alternatively, you can create a conda environment (assume CUDA10.1):
```bash
...
- **GPU (CUDA)**: Use `--gpu 0` (or your GPU device ID)
- **CPU**: Use `--gpu -1`

The code automatically handles device placement based on this flag.
...
```bash
cd data/DATA_NAME
uv run python get_history_graph.py
```

We first pretrain the global model.
```bash
uv run python pretrain.py -d DATA_NAME --gpu 0 --dropout 0.5 --n-hidden 200 --lr 1e-3 --max-epochs 20 --batch-size 1024
```

Then, train the model.
```bash
uv run python train.py -d DATA_NAME --gpu 0 --dropout 0.5 --n-hidden 200 --lr 1e-3 --max-epochs 20 --batch-size 1024
```

We are ready to test!
```bash
uv run python test.py -d DATA_NAME --gpu 0 --n-hidden 200
```

**Note:** Use `--gpu -1` to run on CPU if CUDA is not available.

### Quick Start Example (RAPID dataset)

For a quick test on the small RAPID dataset (54 entities, 8 relations):
...
# Pretrain
uv run python pretrain.py -d RAPID --gpu -1 --n-hidden 100 --max-epochs 5

# Train
uv run python train.py -d RAPID --gpu -1 --n-hidden 100 --lr 1e-3 --max-epochs 5 --batch-size 512 --num-k 54

# Test
uv run python test.py -d RAPID --gpu -1 --n-hidden 100 --num-k 54
```

**Note:** For small datasets like RAPID, set `--num-k` to be at most the number of entities (54 for RAPID).
