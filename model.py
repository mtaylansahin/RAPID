...
class RENet(nn.Module):
    ...
    def forward(self, triplets, s_hist, o_hist, graph_dict, subject=True):
        ...
        hist_len = to_device(torch.LongTensor(list(map(len, hist[0]))))
        ...
        s_h = s_h.squeeze()
        s_h = torch.cat((s_h, to_device(torch.zeros(len(s) - len(s_h), self.h_dim))), dim=0)
        ob_pred = self.linear(
        ...
        s_q = s_q.squeeze()
        s_q = torch.cat((s_q, to_device(torch.zeros(len(s) - len(s_q), self.h_dim))), dim=0)
    ...
    def pred_r_rank2(self, s, r, subject=True):
        ...
        if len(s_history[0]) == 0:
            s_h = to_device(torch.zeros(self.num_rels, self.h_dim))
            s_q = to_device(torch.zeros(self.num_rels, self.h_dim))
        else:
            ...
            if s_packed_input is None:
                s_h = to_device(torch.zeros(len(s), self.h_dim))
                s_q = to_device(torch.zeros(len(s), self.h_dim))
            else:
                tt, s_h = self.encoder(s_packed_input)
                s_h = s_h.squeeze()
                s_h = torch.cat((s_h, to_device(torch.zeros(len(s) - len(s_h), self.h_dim))), dim=0)
    ...
    def predict(self, triplet, s_hist, o_hist, global_model):
        ...
        if len(s_hist[0]) == 0 or len(self.s_hist_test[s]) == 0:
            s_h = to_device(torch.zeros(self.h_dim))
        else:
        ...
        if len(o_hist[0]) == 0 or len(self.o_hist_test[o]) == 0:
            o_h = to_device(torch.zeros(self.h_dim))
        else:
