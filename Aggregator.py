...
class RGCNAggregator_global(nn.Module):
    ...
    def forward(self, t_list, ent_embeds, graph_dict, reverse):
        ...
        embed_seq_tensor = to_device(torch.zeros(len(len_non_zero), self.seq_len, self.h_dim))
...
class RGCNAggregator(nn.Module):
    ...
    def forward(self, s_hist, s, r, ent_embeds, rel_embeds, graph_dict, global_emb, reverse):
        ...
        if length == 0:
            ...
            if g is None:
                ...
                s_embed_seq_tensor = to_device(torch.zeros(len(s_len_non_zero), self.seq_len, 4 * self.h_dim))

                s_embed_seq_tensor_r = to_device(torch.zeros(len(s_len_non_zero), self.seq_len, 3 * self.h_dim))
    ...
    def predict(self, s_history, s, r, ent_embeds, rel_embeds, graph_dict, global_emb, reverse):
        ...
        inp = to_device(torch.zeros(len(s_hist), 4 * self.h_dim))
        ...
        inp_r = to_device(torch.zeros(len(s_hist), 3 * self.h_dim))
...
class MeanAggregator(nn.Module):
    ...
    def forward(self, s_hist, s, r, ent_embeds, rel_embeds):
        ...
        embeds_split = torch.split(embeds_mean, s_len_non_zero.tolist())
        s_embed_seq_tensor = to_device(torch.zeros(len(s_len_non_zero), self.seq_len, 2 * self.h_dim))
    ...
    def predict(self, s_history, s, r, ent_embeds, rel_embeds):
        inp = to_device(torch.zeros(len(s_history), 2 * self.h_dim))
...
class AttnAggregator(nn.Module):
    ...
    def forward(self, s_hist, s, r, ent_embeds, rel_embeds):
        ...
        s_embed_seq_tensor = to_device(torch.zeros(len(s_len_non_zero), self.seq_len, 3 * self.h_dim))
    ...
    def predict(self, s_history, s, r, ent_embeds, rel_embeds):
        inp = to_device(torch.zeros(len(s_history), 3 * self.h_dim))
