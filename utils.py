...
# Global flag to control CUDA usage - set by main scripts
USE_CUDA = False
...
def cuda(tensor):
    if USE_CUDA and tensor.device == torch.device('cpu'):
        return tensor.cuda()
    return tensor
...
def get_sorted_s_r_embed_rgcn(s_hist_data, s, r, ent_embeds, graph_dict, global_emb):
    ...
    g_list = [g.to(get_device()) for g in g_list]  
    ...
    global_emb_list = to_device(torch.cat(global_emb_list, dim=0))
...
def soft_cross_entropy(pred, soft_targets):
    ...
    pred = to_device(pred.type('torch.DoubleTensor'))
